...
class PlanningBenchmark(metaclass=Singleton):
    ...
    def get_stats(self, domain_name, stat="time", xaxis="action", approach=None):
        if stat == "time":
            y = [
                instance.time
                for k, instance in self.stat_instances[domain_name].items()
            ]
        elif stat == "nodes":
            y = [
                instance.nodes
        ...
        if xaxis == "action":
            x = [
                instance.action_space
                for k, instance in self.stat_instances[domain_name].items()
            ]
        elif xaxis == "problem":
            x = [
                instance.problem_name[instance.problem_name.rfind("/") + 1 :]
                for k, instance in self.stat_instances[domain_name].items()
            ]
    ...
    def plot_stat(
        self, domain_name, stat="time", xaxis="action", label="", approach=None
    ):
        if label == "":
