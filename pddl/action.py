...
class Action:
    ...
    def __str__(self):
        return (
            ...
            + str(self.parameters)
            + "\n  positive_preconditions: "
            + str(list(self.positive_preconditions))
            + "\n  negative_preconditions: "
            + str(list(self.negative_preconditions))
            + "\n  add_effects: "
            + str(list(self.add_effects))
            + "\n  del_effects: "
            + str(list(self.del_effects))
            + "\n"
        )
    ...
    def __hash__(self):
        ...
        )  # This should work even in the ground case.
    ...
    def is_mutex(self, an_action):
        if self.positive_preconditions.intersection(
            an_action.negative_preconditions | an_action.del_effects
        ):
            return True
        if self.negative_preconditions.intersection(
            an_action.positive_preconditions | an_action.add_effects
        ):
            return True
        if self.add_effects.intersection(
            an_action.negative_preconditions | an_action.del_effects
        ):
            return True
        if self.del_effects.intersection(
            an_action.positive_preconditions | an_action.add_effects
        ):
            return True
